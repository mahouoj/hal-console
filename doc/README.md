...
### 3-影
#### ブロックの影
左後からの光源をイメージして、隣接するブロックを基づいて影を表示
...
##### 回転視点
レイと表面の交点から、そのピクセルが（プレイヤーから見て）左・上・左上にあたるかを判定する

該当する方向にブロックが存在する場合、影を表示する。  
スクリーンの右方向・前方向に対応するワールド座標のうち、絶対値の大きい方の軸を選んで、隣接ブロックの添え字を計算する
<p>
<img alt="implement_shadow_rotate" src="images/implement_shadow_rotate.png" height="150">
</p>

#### オブジェクトの影
常に1ピクセル下に表示する（プレーヤーがジャンプする時以外）

##### 正面視点
プレーヤーが地面から離れる場合があるため、プレーヤーの下にあるブロックを探索して影を表示する
前後関係を処理するため、Ｚバファを使っている

##### 回転視点
前の行の状態を保持し、現在のピクセル直前の行に物体が存在する場合に影を表示する
